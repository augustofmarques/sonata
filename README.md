...
**Status:** Consolidated Working Draft 0.5
...
- [Containers](system/containers.md)
...
- [OOBE / First Use](patterns/oobe.md)
...
- Typography is a system-level architecture, not locked to one font family. Recommended default: Red Hat Display / Text / Mono, with multilingual fallback strategy, while remaining replaceable. Mono is an explicit semantic role distinct from numeric typography. Typography now has a v1 candidate concrete scale (Body 16/24 default, plus Heading/Display/Label/Caption/Code ladders, D-214–D-215) with separate scale families rather than one universal ladder (D-217), a Regular/Medium/Bold weight vocabulary (D-219), and Text Scale kept independent from Interface Scale and Density — enlarging text must reflow layout, never shrink to fit (D-220). Optical alignment (perceived alignment over mathematical bounding-box alignment) is now a first-class Sonata principle covering typography and UI generally, not just icons (D-223). See [Typography](expression/typography.md).
...
- Sonata now has a unified State & Interaction Model: domain/application state (saved, syncing, offline, locked...) is explicit and distinct from UI/interaction state (focus, hover, selection...); state persists across four scopes (Persistent/Session/Workspace/Transient); Unavailable/Disabled/Read-only/Locked/Hidden are distinct states, with disabled controls expected to explain why rather than stand mute; Dirty is distinct from Synced, and combinations like `saved+offline` are legitimate; offline/degraded operation, sync conflicts and optimistic updates (with truthful local-vs-confirmed distinction) are all officially modeled; and Operations are first-class entities with identity, lifecycle, feedback and undo relationships. Views sharing data must observe shared semantic state rather than diverge, and Workspace restoration must not resurrect obsolete transient conditions. See [State](system/state.md) and [Operations](system/operations.md).
- Containers (Container, Group, Section, Card, Panel, Stack, Cluster, Grid, Inset/Well, Separator) provide semantic and spatial grouping — a Container is not inherently a visible Surface, and Card is not the universal grouping mechanism. Full-bleed and sticky regions are official capabilities; Indentation is an official structural pattern but must have matching semantic/accessible structure, never a purely visual cue. Rounded floating surfaces (panels, menus, popovers) are explicitly legitimate expression, and translucent/glass material treatment may be used broadly per the existing surface decisions. Material 3 Expressive is now the primary Expression reference (generic Material 3 remains a supporting color/tonal reference), and Apple HIG joins as a secondary reference for platform-aware composition and material surfaces — neither overrides GNOME HIG as Foundation. Tablet/intermediate environments are an explicit adaptive target that may combine touch-first interaction with flexible, desktop-like composition. See [Containers](system/containers.md).
- Token architecture is now four layers (Primitive → Semantic → Component → Rendered UI), with State tokens as independently themeable Component-token variants rather than a fifth abstraction layer — resolving a standing open question in the [Token Resolution Model](system/resolution-model.md). Semantic tokens are the stable design API; primitive values and component-token overrides are controlled/internal. Theme, Preference and Preset are now distinct concepts. User customization has an explicit boundary: it may reshape expression but must not break information architecture, navigation, accessibility or required interaction behavior. External references now carry an explicit authority classification (Normative/Foundational/Reference/Implementation Reference with an Adopted/Adapted/Reference-only/Rejected disposition) in the [Authority Matrix](governance/authority-matrix.md). Sonata also gained official OOBE/First Use and Onboarding patterns: prefer interactive demonstration over lengthy text, focus on meaningful defaults rather than covering the whole application, never force experienced users through it repeatedly, and let onboarding continue contextually after first launch. See [Tokens](system/tokens.md), [OOBE](patterns/oobe.md) and [Onboarding](patterns/onboarding.md).
...
- Exact letter-spacing values, weight-per-family mapping, and locale/script fallback matrix (concrete scale, weight vocabulary and family architecture are decided — D-008, D-214, D-219).
